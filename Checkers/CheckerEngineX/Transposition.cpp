...
void MoveEncoding::encode_move(Move move) {
    ...
    uint8_t dir;
    if ((((move.from & MASK_L3) << 3) == move.to) || (((move.from & MASK_L5) << 5) == move.to)) {
        dir = 0;
    ...
    } else if ((((move.from & MASK_R3) >> 3) == move.to) || (((move.from & MASK_R5) >> 5) == move.to)) {
        dir = 3;
...
MoveEncoding::MoveEncoding(Move m){
	encode_move(m);
}

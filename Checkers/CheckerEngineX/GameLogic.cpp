...
Value qs(bool in_pv, Board &board, Line &pv, Value alpha, Value beta, Ply ply, Depth depth, int last_rev) {
    ...
    if (moves.is_empty()) {
        ...
        if (depth == 0 && board.get_position().has_jumps(~board.get_mover())) {
            return Search::search(in_pv, board, pv, alpha, beta, ply, 1,last_rev,Move{},Move{},Move{});
        } 
    ...
    if (!in_pv && local.depth>1 && std::abs(local.beta)<TB_WIN) {
        ...
        if (board_val >= newBeta) {
            Value value = -Search::search(false, board, line, -(newBeta + 1), -newBeta, local.ply + 1,
                                          newDepth,last_rev,move,local.previous,local.skip_move);
            if (value >= newBeta) {
                val = value;
            }
        }
    ...
    if (val == -INFINITE) {
        if ((in_pv && local.i != 0) || reduction!=0) {
            val = -Search::search(false, board, line, -new_alpha - 1, -new_alpha, local.ply + 1,
                                  new_depth - reduction,last_rev,move,local.previous,local.skip_move);
            if (val > new_alpha) {
                val = -Search::search(in_pv, board, line, -local.beta, -new_alpha, local.ply + 1, new_depth,last_rev,move,local.previous,local.skip_move);
            }
        } else {
            val = -Search::search(in_pv, board, line, -local.beta, -new_alpha, local.ply + 1, new_depth,last_rev,move,local.previous,local.skip_move);
        }
...
void move_loop(bool in_pv, Local &local, Board &board, Line &pv, MoveListe &liste, int last_rev) {
    ...
    if (local.best_score >= local.beta && !board.get_position().has_jumps() && liste.length()>1 && local.skip_move.is_empty()) {

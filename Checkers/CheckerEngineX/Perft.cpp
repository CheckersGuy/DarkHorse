...
size_t Table::get_capacity() { return capacity; }
...
void Table::set_capacity(std::string capa_string) {
  ...
  auto result = std::find_if(size_string.begin(), size_string.end(),
...
size_t Table::probe(Position pos, int depth) {
  ...
  } else if (entries[index][0].WP == pos.WP && entries[index][0].BP == pos.BP &&
             entries[index][0].K == pos.K && entries[index][0].depth == depth) {

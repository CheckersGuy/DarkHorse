...
inline
void
add_capture(const Position &pos, uint32_t orig, uint32_t current, uint32_t captures,
            CallBack &&call_back) {
...
void loop_captures(Position &pos, CallBack &&call_back) {
    ...
    while (king_jumpers) {
        ...
        add_capture<color, KingCapture>(pos, maske, maske, 0, std::forward<CallBack>(call_back));
        mask_bits<color>(pos, maske);
    ...
    while (pawn_jumpers) {
        ...
        add_capture<color, PawnCapture>(pos, maske, maske, 0, std::forward<CallBack>(call_back));

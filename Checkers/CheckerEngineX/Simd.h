...
inline void accum_activation(int16_t *acc, int16_t *out, const int input_size) {
...
inline void accum_activation8(int16_t *acc, int8_t *out, const int input_size) {
  // combined the element-wise multiplicaiton and the square clipped relu
  // maybe its a good idea to split this up at some point
#ifdef AVX256
  ...
  for (auto i = 0; i < input_size / 2; ++i) {
    ...
    int16_t val2 = acc[i + input_size / 2] / 4;
...
inline void square_clipped8(int32_t *input, int8_t *output, const int length) {
// to be continued
#ifdef AVX256
  const int num_chunks = length / 8;
  auto *in = reinterpret_cast<const __m256i *>(input);
  ...
  for (auto i = 0; i < num_chunks / 4; ++i) {
    ...
    auto packed2 = _mm256_packs_epi32(temp2, temp3);
...
inline int flatten(const int16_t *weights, const int16_t *input,
                   const int length) {
#ifdef AVX256
...
inline void vec_add(const int16_t *input, int16_t *out, const int length) {
#ifdef AVX256
...
inline void vec_diff(const int16_t *input, int16_t *out, const int length) {
#ifdef AVX256
  const int num_chunks = length / 16;
  auto accu = reinterpret_cast<__m256i *>(out);
  auto in = reinterpret_cast<const __m256i *>(input);
...
inline int flatten8(const int8_t *weights, const int8_t *input,
  ...
  const int num_chunks = length / 32;
  auto *w = reinterpret_cast<const __m256i *>(weights);
  auto *in = reinterpret_cast<const __m256i *>(input);
  ...
  for (auto i = 0; i < num_chunks; ++i) {
    ...
    auto product = _mm256_madd_epi16(temp, one);
...
inline void test(int length) {
  const int ALIGNMENT = 32;

  auto acc = (int16_t *)std::aligned_alloc(ALIGNMENT, sizeof(int16_t) * length);
  auto out = (int8_t *)std::aligned_alloc(ALIGNMENT, sizeof(int8_t) * length);

  // some random input

  std::mt19937_64 generator(32123);
  std::uniform_int_distribution<int16_t> distrib(4000, 5000);
  std::uniform_int_distribution<int16_t> distrib2(-120, 120);
  for (auto i = 0; i < length; ++i) {
    acc[i] = distrib(generator);
  }
  ...
  for (auto i = 0; i < LOOP_COUNT; ++i) {
    ...
    Simd::accum_activation8(acc, out, length);
  ...
  std::cout << "Average: " << time / LOOP_COUNT << std::endl;
  std::cout << "disregarded allsum: " << all_sum << std::endl;

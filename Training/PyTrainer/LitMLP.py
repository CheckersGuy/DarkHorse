...
class BatchDataSet(torch.utils.data.IterableDataset):
    ...
    def __init__(self, batch_size, buffer_size, file_path, is_patt, is_val_set=False):
...
class NetBatchDataSet(BatchDataSet):
    ...
    def __init__(self, batch_size, buffer_size, file_path, is_val_set=False):
        super(NetBatchDataSet, self).__init__(batch_size, buffer_size, file_path, False, is_val_set)
    ...
    def __next__(self):
        ...
        if not self.is_val_set:
            self.c_lib.get_next_batch(res_p, inp_p)
        else:
            self.c_lib.get_next_val_batch(res_p, inp_p)

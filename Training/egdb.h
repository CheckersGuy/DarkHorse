...
/* Color definitions. */
enum EGDB_COLOR { EGDB_BLACK = 0, EGDB_WHITE = 1 };
...
typedef enum { EGDB_NORMAL = 0, EGDB_ROW_REVERSED } EGDB_BITBOARD_TYPE;
...
typedef struct {
  ...
  unsigned int
      db_not_present_requests; /* requests for positions not in the db */
} EGDB_STATS;
...
typedef struct egdb_driver {
  int(__cdecl *lookup)(struct egdb_driver *handle, EGDB_BITBOARD *position,
                       int color, int cl);
 ...
 * The return value is non-zero if no database is found.  In this case the
 * values for egdb_type and max_pieces are undefined on return.
 */
EGDB_API int __cdecl egdb_identify(const char *directory, EGDB_TYPE *egdb_type,
                                   int *max_pieces);

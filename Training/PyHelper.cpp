...
extern "C" int init_streamer(size_t buffer_size, size_t batch_size,
  ...
  std::cout << "NumPositions in training set: "
            << streamer->get_streamer().get_num_positions() << std::endl;
...
extern "C" int init_val_streamer(size_t buffer_size, size_t batch_size,
  ...
  if (val_streamer.get() == nullptr) {
    val_streamer =
        std::make_unique<BatchProvider>(path, buffer_size, batch_size);
  }
  val_streamer->get_streamer().set_shuffle(false);
  std::cout << "NumPositions in validation set: "
            << val_streamer->get_streamer().get_num_positions() << std::endl;
...
extern "C" void print_position(uint32_t white_men, uint32_t black_men,
                               uint32_t kings) {
  Position pos;
  pos.WP = white_men;
  pos.BP = black_men;
